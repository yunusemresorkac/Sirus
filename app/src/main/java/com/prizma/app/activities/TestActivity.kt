...
import com.prizma.app.databinding.ActivityTestBinding
    ...
    private fun loadQuestions() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // Kullanıcının çözdüğü soruların arasında olmayan soruları Firestore'dan al
                val allQuestionsSnapshot = FirebaseFirestore.getInstance().collection("Questions")
                    .orderBy("questionId")
                    .whereEqualTo("status", Constants.ACTIVE_QUESTION)
                    .get()
                    .await()
                ...
                for (document in allQuestionsSnapshot.documents) {
                    ...
                    if (question?.questionId != null && question.status == Constants.ACTIVE_QUESTION && !solvedQuestionIds.contains(question.questionId)) {
                        ...
                        println("başta eklenen ${remainingQuestions.size}")

                    }

                    if (remainingQuestions.size >= 7) {
                        break // 7 soru elde ettiğimizde döngüden çık
                    }
                }
